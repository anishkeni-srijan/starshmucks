...
class EditdetailsBloc extends Bloc<EditdetailsEvent, EditdetailsState> {
  EditdetailsBloc() : super(EditdetailsInitialState()) {
    ...
    on<EditdetailsemailChangedEvent>(
            (event, emit) {
//user exists
          if (event.emailvalue != '') {
            emit(EditdetailsValidState("all good"));
              ...
              event.emailvalue.trim().length < 4) {
            emit(
    ...
    on<EditdetailsNameChangedEvent>(
            (event, emit) {
//user exists
          if (event.namevalue == '' ||
              event.namevalue.trim().length < 4 ) {
            emit(
    ...
    on<EditdetailsNumberChangedEvent>(
        ...
        if (event.phnvalue == null || event.phnvalue.trim().isEmpty) {
          ...
          emit(
            EditdetailsErrorState("Your phone number must be at least 4 digits"),
          );
        }
