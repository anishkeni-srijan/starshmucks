  ...
  ResetpasswordBloc() : super(ResetpasswordInitialState()) {
    ...
    on<PasswordChangedEvent>((event, emit) {
      if (event.newpassword == null || event.newpassword.trim().isEmpty) {
        emit(ResetpasswordErrorState("Please enter a password"));
      } else if (event.newpassword.trim().length < 4) {
        emit(ResetpasswordErrorState("Your password must be at least 4 Characters"));
      }
      // Return null if the entered username is valid
      else emit(ResetpasswordValidState('Done! Now let\'s confirm your password'));
    });
    on<ConfirmpasswordChangedEvent>((event, emit) {
      ...
      // Return null if the entered username is valid
      else emit(ResetpasswordValidState('All Set!'));
    });

    //works if login is valid
    on<ResetpasswordSubmittedEvent>((event, emit) {
